...
function renderTabs(args, body) {
  ...
  tabs.forEach((tab, index) => {
    ...
    tabButtons += `
<button type="button" role="tab" aria-selected="${isFirst ? 'true' : 'false'}" aria-controls="${tabId}-panel-${index}" id="${tabId}-tab-${index}" class="docboot-tab-btn px-4 py-2 text-xs transition-all border-b-2 -mb-px rounded-t-lg select-none ${activeClass}" data-tab-index="${index}" data-tab-label="${escapeHtml(tab.label)}">
${escapeHtml(tab.label)}
</button>`;

    const parsedContent = marked.parse(tab.content);
    tabPanels += `
<div role="tabpanel" id="${tabId}-panel-${index}" aria-labelledby="${tabId}-tab-${index}" class="docboot-tab-panel p-4 text-sm ${hiddenClass}" data-tab-index="${index}">
...
<div class="docboot-tab-list flex border-b border-border/60 bg-muted/30 px-2 pt-2 gap-1 overflow-x-auto" role="tablist">
${tabButtons}
</div>
...
function renderCodeGroup(args, body) {
  ...
  tabs.forEach((tab, index) => {
    ...
    tabButtons += `
<button type="button" role="tab" aria-selected="${isFirst ? 'true' : 'false'}" aria-controls="${tabId}-panel-${index}" id="${tabId}-tab-${index}" class="docboot-tab-btn px-3.5 py-2 text-xs transition-all border-b-2 -mb-px rounded-t-md select-none ${activeClass}" data-tab-index="${index}" data-tab-label="${escapeHtml(tab.label)}">
${escapeHtml(tab.label)}
</button>`;

    const renderedCode = renderCodeBlock(tab.code, tab.lang);
    tabPanels += `
<div role="tabpanel" id="${tabId}-panel-${index}" aria-labelledby="${tabId}-tab-${index}" class="docboot-tab-panel ${hiddenClass}" data-tab-index="${index}">
${renderedCode}
</div>`;
  });
